...
| OpenWrt в заводской разметке | OpenWrt с разметкой UBI | **нельзя**, переход односторонний |
| OpenWrt с разметкой UBI | OpenWrt в заводской разметке | **нельзя** |

Проще говоря: разметку выбирают один раз, при уходе с заводской прошивки. Дальше вы обновляетесь внутри выбранной, а не прыгаете между ними. Если попытаться, UrsusFlasher откажет до начала записи и объяснит причину.
