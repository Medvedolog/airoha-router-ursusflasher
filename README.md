...
### Это обычный U-Boot, а не что-то своё

База — **U-Boot v2026.07** плюс патчи OpenWrt/Airoha под эту плату и собственный слой UrsusBoot: страница восстановления, политика записи и проверок. Никакого форка «с нуля» тут нет, и знакомые по U-Boot вещи работают как обычно.

**Родная командная строка доступна из двух мест.** По UART это привычный промпт: строка уходит прямо в `run_command()`. В веб-интерфейсе есть вкладка «Консоль U-Boot», и это **та же самая командная строка** — без белого списка и без блокировки операций записи. В исходниках она так и подписана: *the real U-Boot command line*.

Из этого следует и предупреждение: консоль ничем вас не ограничивает. Проверки и подтверждения, описанные ниже, охраняют кнопки веб-интерфейса, а не то, что вы наберёте руками.

**Что доступно.** В сборке включено 67 команд U-Boot — работа с MTD, UBI и UBIFS, сеть с `tftpboot`, `ping`, `dhcp`, `wget`, `mii`, `mdio`, полный набор для environment, `bootm`, `fdt`, `gpio`, `led`, `button`, `hash`, `crc32`, распаковка `lzma`/`unzip`.

**Что выключено.** Ещё 93 опции команд собраны без поддержки, и это стоит сказать честно, а не отделываться словами «почти ничего не вырезали». Убраны файловые системы, которых на этом пути нет (`ext2/4`, `fat`, `squashfs`, `btrfs`, `zfs`), шины и носители, которых нет на плате (`pci`, `ide`, `sf`/`spi`, `onenand`), и часть отладочных удобств (`memtest`, `md5sum`, `sha1sum`, `date`, `bootmenu`, `history`, `cat`, `xxd`, `nfs`). Всё, что нужно для загрузки, сети, работы с NAND и восстановления, на месте.
