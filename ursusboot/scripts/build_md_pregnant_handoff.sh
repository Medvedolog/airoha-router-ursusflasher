...
sha256sum u-boot.bin "$OUT/ursusboot-md-${VERSION}.linuximg" | tee "$OUT/SHA256SUMS"
printf '%s\n'     "UrsusBoot ${VERSION}"     "MODE=PREGNANT_HANDOFF"     "PERSISTENCE_TARGET=NONE"     "RUNTIME_DEST=${RUNTIME_DEST}"     "STOCK_KERNEL_LOAD=${STOCK_KERNEL_LOAD}"     "TEXT_BASE=${TEXT_BASE}"     "SOURCE_DATE_EPOCH=${RELEASE_EPOCH}" > "$OUT/PREGNANT-HANDOFF-BUILD_INFO.txt"
