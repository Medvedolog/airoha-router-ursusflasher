...
grep -q '^CONFIG_PSCI_RESET=yfor sym in CONFIG_NET_LWIP CONFIG_CMD_PING CONFIG_CMD_DHCP CONFIG_CMD_DNS CONFIG_CMD_SNTP CONFIG_CMD_TFTPBOOT CONFIG_CMD_WGET CONFIG_CMD_MTD; do
...
echo "Artifacts: $OUT"
 .config || { echo 'PREGNANT1: PSCI reset backend missing' >&2; exit 1; }
grep -q '^CONFIG_RESET_AIROHA=yfor sym in CONFIG_NET_LWIP CONFIG_CMD_PING CONFIG_CMD_DHCP CONFIG_CMD_DNS CONFIG_CMD_SNTP CONFIG_CMD_TFTPBOOT CONFIG_CMD_WGET CONFIG_CMD_MTD; do
    if grep -q "^${sym}=y$" .config; then
        echo "PREGNANT1: forbidden transient handoff capability ${sym}=y" >&2
        exit 1
    fi
...
echo "MD_PREGNANT_HANDOFF_BUILD=PASS"
echo "Artifacts: $OUT"
 .config || { echo 'PREGNANT1: Airoha reset backend missing' >&2; exit 1; }
