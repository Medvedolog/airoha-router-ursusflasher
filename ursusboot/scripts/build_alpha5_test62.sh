...
WORK=${URSUS_BUILD_DIR:-"$ROOT/work/alpha5-test62-rebuild"}
...
python3 - "$OUT/ursusboot-md-0.1.0-alpha5-UBIUX1-TEST62-update.fip" <<'PYQA'
import struct,sys
...
assert len(d) == 0x7b000, hex(len(d))
print(f'FIP_BOUNDARY_QA=PASS file_end=0x{len(d):x} physical_end=0x7b800 cert_first=0x77800 protected_env=0x7c000 nt_off=0x{nt[0]:x} nt_size={nt[1]} nt_margin={0x77800-(nt[0]+nt[1])}')
PYQA
