...
$root = Join-Path (Resolve-Path (Join-Path $PSScriptRoot "..\results")) "xg140-full-backup-$stamp"
New-Item -ItemType Directory -Force -Path $root | Out-Null
$sshOpts = @('-o','StrictHostKeyChecking=no','-o','UserKnownHostsFile=NUL','-o','ConnectTimeout=5','root@'+$RouterIP)
function SshText([string]$cmd) {
  $out = & ssh @sshOpts $cmd 2>&1
  if ($LASTEXITCODE -ne 0) { throw "SSH command failed: $cmd`n$out" }
  return ($out -join "`n")
}
...
foreach ($line in ($mtd -split "`n")) {
  if ($line -match '^mtd(\d+):\s+([0-9a-fA-F]+)\s+([0-9a-fA-F]+)\s+"([^"]+)"') {
    $parts += [pscustomobject]@{ Index=[int]$Matches[1]; Size=[Convert]::ToInt64($Matches[2],16); Erase=[Convert]::ToInt64($Matches[3],16); Name=$Matches[4] }
  }
}
...
foreach ($p in $parts) {
  ...
  foreach ($a in $sshOpts) { [void]$psi.ArgumentList.Add($a) }
  [void]$psi.ArgumentList.Add(("gzip -1 < /dev/mtd{0}" -f $p.Index))
  ...
  $proc.WaitForExit()
  if ($proc.ExitCode -ne 0) { throw "Backup failed for mtd$($p.Index): $err" }
  ...
  [Int64]$rawSize = 0
  try {
    while (($n = $gzs.Read($buf,0,$buf.Length)) -gt 0) { [void]$sha.TransformBlock($buf,0,$n,$null,0); $rawSize += $n }
    [void]$sha.TransformFinalBlock([byte[]]::new(0),0,0)
    $localHash = -join ($sha.Hash | ForEach-Object { $_.ToString('x2') })
  } finally { $sha.Dispose(); $gzs.Dispose(); $in.Dispose() }
