...
APP_VERSION = "0.1.0-md-lab1"
BUILD_TAG = "ursusflasher-0.1.0-md-lab1-proven-port"
...
class _ConsoleTee:
    def __init__(self, console, files):
        ...
        self.files = files
        self._session_line_start = True

    def _write_timestamped_session(self, fh, clean: str) -> None:
        # Console must stay byte-clean for UART/XMODEM.  The detailed session
        # log keeps one timestamp per logical line, even when UART arrives in
        # tiny chunks ("P" + "ress x" etc.).
        if not clean:
            return
        pos = 0
        while pos < len(clean):
            nl = clean.find("\n", pos)
            end = len(clean) if nl < 0 else nl + 1
            part = clean[pos:end]
            if self._session_line_start and part not in ("\n", "\r\n"):
                fh.write(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] ")
            fh.write(part)
            self._session_line_start = part.endswith("\n")
            pos = end
    ...
    def write(self, text):
        ...
        if self.files:
            self._write_timestamped_session(self.files[0], clean)
            for fh in self.files[1:]:
                fh.write(clean)
        return written
...
# UrsusBoot WebFailsafe terminal palette. Keep this aligned with
# webfailsafe-alpha4.html, not generic ANSI cyan/magenta.
_ANSI_AMBER = "1;38;2;217;154;77"     # #D99A4D
_ANSI_AMBER2 = "1;38;2;200;135;58"    # #C8873A
_ANSI_SAND = "1;38;2;239;192;121"     # #EFC079
_ANSI_INK = "38;2;242;232;218"         # #F2E8DA
_ANSI_MUTED = "38;2;168;145;121"       # #A89179
_ANSI_OK = "1;38;2;124;196;147"        # #7CC493
_ANSI_BAD = "1;38;2;232;131;122"       # #E8837A
...
def colorize_text(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        ...
        if re.match(r"^-{6,}\s*STAGE(?:_|-)", upper) or re.match(r"^-{6,}\s*COMPLETE", upper):
            body = _color(body, _ANSI_AMBER)
        elif re.match(r"^(?:ERROR(?: / OSHIBKA)?|ОШИБКА|CRITICAL FAILURE):", upper) or upper.startswith(("[ERROR]", "[FAIL]", "[ОШИБКА]", "[СТОП]")):
            body = _color(body, _ANSI_BAD)
        elif upper.startswith(("[ПРЕДУПРЕЖДЕНИЕ]", "[WARNING]", "[ВНИМАНИЕ]", "[СДЕЛАЙТЕ]")):
            body = _color(body, _ANSI_SAND)
        elif upper.startswith(("[OK]", "[ГОТОВО]")) or re.match(r"^OK:", upper) or upper.startswith(("PREFLIGHT PASSED", "ГОТОВО:")):
            body = _color(body, _ANSI_OK)
            ...
            body = _color(body, _ANSI_OK)
        elif upper.startswith(("[WAIT]", "[ЖДУ]", "[TRANSFER]", "[FLASH]", "[VERIFY]", "[TFTP]", "[SCP]", "[TCP/NC]", "[NET]", "[STEP ", "[ШАГ]", "[INFO]", "[ИНФО]", "[UART]", "[XMODEM]")):
            body = _color(body, _ANSI_AMBER2 if upper.startswith(("[WAIT]", "[ЖДУ]")) else (_ANSI_SAND if upper.startswith(("[UART]", "[XMODEM]")) else _ANSI_AMBER))
        elif upper.startswith("[INPUT]"):
            body = _color(body, _ANSI_SAND)
        elif upper.startswith("[PATH]"):
            body = _color(body, _ANSI_AMBER)
        elif upper.startswith("[TECH]"):
            body = _color(body, _ANSI_MUTED)
        elif upper.startswith("[LOG]"):
            body = _color(body, _ANSI_AMBER2)
...
def print_app_banner() -> None:
    ...
    bear = RichText()
    bear.append("       ʕ•ᴥ•ʔ\n", style="bold rgb(200,135,58)")
    bear.append("      /|   |\\\n", style="rgb(200,135,58)")
    bear.append("     /_|___|_\\", style="rgb(200,135,58)")
    console.print(bear)
    title = RichText("Nokia Router MedveFlasher", style="bold rgb(217,154,77)")
    title.append("  •  ", style="rgb(168,145,121)")
    title.append(APP_VERSION, style="bold rgb(124,196,147)")
    console.print(title)
    console.print(RichText(BUILD_TAG, style="rgb(168,145,121)"))
...
def _localized_print(*args, **kwargs):
    ...
    # No timestamps in the live console: UART/XMODEM output must remain readable
    # and must not be split by per-chunk prefixes.  _ConsoleTee adds timestamps
    # only to the detailed session log.
...
def _print_install_backup_validation(profile: InstallProfile, backup_dir: Path, validation: dict) -> None:
    ...
    if family == "MD":
        factory_mac = _backup_md_factory_mac(backup_dir)
        if factory_mac != "UNKNOWN":
            print(tr(
                f"[OK] Backup factory LAN MAC: {factory_mac} (mtd7/ri@0x3e).",
                f"[OK] Backup factory LAN MAC: {factory_mac} (mtd7/ri@0x3e).",
            ))
            meta = _read_backup_device_mac_fields(backup_dir / "DEVICE_MAC.txt")
            legacy_mac = meta.get("primary_mac", "").lower()
            if _MAC_RE.fullmatch(legacy_mac) and legacy_mac != factory_mac:
                print(tr(
                    f"[WARNING] DEVICE_MAC.txt содержит legacy/runtime MAC {legacy_mac}; для MD identity он игнорируется.",
                    f"[WARNING] DEVICE_MAC.txt contains legacy/runtime MAC {legacy_mac}; it is ignored for MD identity.",
                ))
        else:
            print(tr(
                "[WARNING] Не удалось извлечь MD factory MAC из mtd7/ri@0x3e.",
                "[WARNING] Could not extract the MD factory MAC from mtd7/ri@0x3e.",
            ))
    else:
...
def _validated_mac_bytes(raw: bytes) -> str:
    if len(raw) != 6:
        return "UNKNOWN"
    if raw in (b"\x00" * 6, b"\xff" * 6):
        return "UNKNOWN"
    if raw[0] & 0x01:
        return "UNKNOWN"
...
def _backup_md_factory_mac(directory: Path) -> str:
    """Authoritative MD identity: six factory LAN-MAC bytes at mtd7/ri offset 0x3e."""
    try:
        path = find_dump(directory, 7)
        if path is None:
            return "UNKNOWN"
        raw = read_dump(path)
    ...
    if len(raw) < 0x44:
        return "UNKNOWN"
    return _validated_mac_bytes(raw[0x3E:0x44])


def _stock_md_factory_mac(telnet: Telnet) -> str:
    """Read factory LAN MAC from stock MD ri@0x3e without changing device state."""
    command = (
        "if grep -q '^mtd7:.*\"ri\"$' /proc/mtd 2>/dev/null; then "
        "d=/dev/mtd7ro; [ -r \"$d\" ] || d=/dev/mtd7; "
        "h=''; "
        "if [ -r \"$d\" ]; then "
        "if busybox hexdump -v -e '1/1 \"%02x\"' /dev/null >/dev/null 2>&1; then "
        "h=$(dd if=\"$d\" bs=1 skip=62 count=6 2>/dev/null | busybox hexdump -v -e '1/1 \"%02x\"'); "
        "elif command -v hexdump >/dev/null 2>&1; then "
        "h=$(dd if=\"$d\" bs=1 skip=62 count=6 2>/dev/null | hexdump -v -e '1/1 \"%02x\"'); "
        "elif command -v od >/dev/null 2>&1; then "
        "h=$(dd if=\"$d\" bs=1 skip=62 count=6 2>/dev/null | od -An -tx1 | tr -d '[:space:]'); "
        "fi; fi; printf '__NOKIA_RIMAC__%s__\\n' \"$h\"; fi"
    )
    ...
    if not values:
        return "UNKNOWN"
    hexmac = values[-1].strip().lower()
    if not re.fullmatch(r"[0-9a-f]{12}", hexmac):
        return "UNKNOWN"
    return _validated_mac_bytes(bytes.fromhex(hexmac))




_RI_IDENTITY_FIELDS = (
    "Format", "MfrID", "Factorycode", "HardwareVersion", "ICS", "YPSerialNum",
    "CleiCode", "Mnemonic", "ProgDate", "MACAddress", "G984Serial", "PartNumber",
    "Variant", "OperatorID", "CountryID",
)


def _clean_ri_identity_value(value: str, max_len: int = 96) -> str:
    value = "".join(ch for ch in value.strip() if 0x20 <= ord(ch) < 0x7f)
    return value[:max_len]


def _parse_ritool_identity(text: str) -> dict[str, str]:
    """Extract only non-secret factory identity fields from `ritool dump` output."""
    wanted = {key.lower(): key for key in _RI_IDENTITY_FIELDS}
    out: dict[str, str] = {}
    for raw in text.splitlines():
        line = raw.strip()
        if not line:
            continue
        line = re.sub(r"^the\s+", "", line, flags=re.I)
        if ":" not in line:
            continue
        key, value = line.split(":", 1)
        canonical = wanted.get(key.strip().lower())
        if canonical:
            out[canonical] = _clean_ri_identity_value(value)
    return out




def _stock_md_fixed_identity(telnet: Telnet) -> dict[str, str]:
    """Read only the hardware-confirmed non-secret MD identity fields from RI.

    The real XG-040G-MD RI layout stores the 12-character GPON/box serial at
    ri@0x1a and the 4-byte G.984 serial suffix at ri@0x58.  We read only those
    exact byte ranges (never the surrounding RI area, which may contain
    credentials), then require the ASCII serial and binary suffix to agree.
    """
    command = (
        "if grep -q '^mtd7:.*\"ri\"$' /proc/mtd 2>/dev/null; then "
        "d=/dev/mtd7ro; [ -r \"$d\" ] || d=/dev/mtd7; s=''; g=''; "
        "if [ -r \"$d\" ]; then "
        "if busybox hexdump -v -e '1/1 \"%02x\"' /dev/null >/dev/null 2>&1; then "
        "s=$(dd if=\"$d\" bs=1 skip=26 count=12 2>/dev/null | busybox hexdump -v -e '1/1 \"%02x\"'); "
        "g=$(dd if=\"$d\" bs=1 skip=88 count=4 2>/dev/null | busybox hexdump -v -e '1/1 \"%02x\"'); "
        "elif command -v hexdump >/dev/null 2>&1; then "
        "s=$(dd if=\"$d\" bs=1 skip=26 count=12 2>/dev/null | hexdump -v -e '1/1 \"%02x\"'); "
        "g=$(dd if=\"$d\" bs=1 skip=88 count=4 2>/dev/null | hexdump -v -e '1/1 \"%02x\"'); "
        "elif command -v od >/dev/null 2>&1; then "
        "s=$(dd if=\"$d\" bs=1 skip=26 count=12 2>/dev/null | od -An -tx1 | tr -d '[:space:]'); "
        "g=$(dd if=\"$d\" bs=1 skip=88 count=4 2>/dev/null | od -An -tx1 | tr -d '[:space:]'); "
        "fi; fi; printf '__NOKIA_RIID__%s:%s__\\n' \"$s\" \"$g\"; fi"
    )
    rc, text = telnet.command(command, timeout=20, echo=False)
    if rc:
        return {}
    values = _runtime_marker_values(text, "__NOKIA_RIID__")
    if not values or ":" not in values[-1]:
        return {}
    serial_hex, suffix_hex = (part.strip().lower() for part in values[-1].split(":", 1))
    if not re.fullmatch(r"[0-9a-f]{24}", serial_hex) or not re.fullmatch(r"[0-9a-f]{8}", suffix_hex):
        return {}
    try:
        serial = bytes.fromhex(serial_hex).decode("ascii").upper()
    except (ValueError, UnicodeDecodeError):
        return {}
    if not re.fullmatch(r"[A-Z]{4}[0-9A-F]{8}", serial):
        return {}
    # Strong consistency check: RI binary G.984 suffix must match the ASCII serial.
    if serial[4:].lower() != suffix_hex:
        return {}
    return {
        "YPSerialNum": serial,
        "G984Serial": serial,
        "MfrID": serial[:4],
        "_SerialSource": "ri@0x1a+g984@0x58",
    }


def _stock_ri_identity(telnet: Telnet) -> dict[str, str]:
    """Read factory identity from the stock RI database without modifying it."""
    rc, text = telnet.command(
        "if command -v ritool >/dev/null 2>&1; then ritool dump 2>/dev/null; else echo __NO_RITOOL__; fi",
        timeout=30, echo=False,
    )
    if rc or "__NO_RITOOL__" in text:
        return {}
    return _parse_ritool_identity(text)


def _write_backup_device_identity(destination: Path, telnet: Telnet, model_name: str, family: str) -> dict[str, str]:
    """Persist non-secret factory identity alongside the full stock backup.

    For MD, hardware-confirmed fixed RI byte ranges provide the authoritative
    serial/G.984 identity; vendor `ritool` remains a secondary metadata source.
    Raw RI/ritool output is deliberately NOT saved because surrounding RI data
    may contain passwords/LOID and other secrets.
    """
    mac_fields = _read_backup_device_mac_fields(destination / "DEVICE_MAC.txt")
    ...
    ri = _stock_ri_identity(telnet) if family == "md" else {}
    fixed_ri = _stock_md_fixed_identity(telnet) if family == "md" else {}
    if fixed_ri:
        # Fixed-offset RI fields are hardware-confirmed and take precedence over
        # vendor ritool output, which is absent/incomplete on some stock builds.
        ri.update(fixed_ri)
    yp = _clean_ri_identity_value(ri.get("YPSerialNum", ""))
    g984 = _clean_ri_identity_value(ri.get("G984Serial", ""))
    mfr = _clean_ri_identity_value(ri.get("MfrID", ""))
    serial = yp
    serial_source = _clean_ri_identity_value(ri.get("_SerialSource", "")) or "ri/ritool:YPSerialNum"
    if not serial and mfr and g984:
        serial = (mfr + g984).upper()
        serial_source = "ri/ritool:MfrID+G984Serial"
    if not serial:
        serial = "UNKNOWN"
    ...
    identity.update({
        ...
        "part_number": _clean_ri_identity_value(ri.get("PartNumber", "")) or "UNKNOWN",
        "hardware_version": _clean_ri_identity_value(ri.get("HardwareVersion", "")) or "UNKNOWN",
        "mnemonic": _clean_ri_identity_value(ri.get("Mnemonic", "")) or "UNKNOWN",
        "program_date": _clean_ri_identity_value(ri.get("ProgDate", "")) or "UNKNOWN",
        "ri_mac": (
            identity["primary_mac"].lower()
            if family == "md" and identity.get("primary_mac_source") == "ri@0x3e" and _MAC_RE.fullmatch(identity.get("primary_mac", "").lower())
            else (_clean_ri_identity_value(ri.get("MACAddress", "")).lower() or "UNKNOWN")
        ),
    })
    ...
    write_text(destination / "DEVICE_IDENTITY.txt", "\n".join(lines) + "\n")
    write_text(destination / "DEVICE_IDENTITY.json", json.dumps(identity, ensure_ascii=False, indent=2) + "\n")
    if serial == "UNKNOWN":
        print(tr(
            "[ВНИМАНИЕ] Не удалось прочитать заводской серийный номер из RI. Копирование продолжаю; в DEVICE_IDENTITY будет serial_number=UNKNOWN.",
            "[WARNING] Factory serial could not be read from RI; backup continues and DEVICE_IDENTITY contains serial_number=UNKNOWN.",
        ))
    else:
        print(tr(
            f"[ГОТОВО] Заводской серийный номер: {serial} ({serial_source}); G984Serial={identity['g984_serial']}.",
            f"[PASS] Factory serial: {serial} ({serial_source}); G984Serial={identity['g984_serial']}.",
        ))
    return identity

def _read_backup_device_mac_fields(path: Path) -> dict[str, str]:
    if not path.is_file():
        return {}
    values: dict[str, str] = {}
    try:
...
def _read_backup_device_mac(path: Path) -> tuple[str, str] | None:
    values = _read_backup_device_mac_fields(path)
...
def _write_backup_device_mac(destination: Path, telnet: Telnet, model_name: str, family: str) -> tuple[str, str]:
    ...
    if family == "md":
        ...
        mac = _stock_md_factory_mac(telnet)
        if mac == "UNKNOWN":
            raise Error(tr(
                "Не удалось прочитать factory LAN MAC из stock mtd7/ri@0x3e; MD backup остановлен fail-closed.",
                "Could not read the factory LAN MAC from stock mtd7/ri@0x3e; MD backup stopped fail-closed.",
            ))
    else:
        interface, mac = _backup_primary_mac(macs)

    existing_fields = _read_backup_device_mac_fields(destination / "DEVICE_MAC.txt")
    existing = _read_backup_device_mac(destination / "DEVICE_MAC.txt")
    if existing is not None and mac != "UNKNOWN" and existing[1] != mac:
        legacy_ok = False
        if family == "md" and existing_fields.get("primary_mac_source") != "ri@0x3e":
            backup_factory = _backup_md_factory_mac(destination)
            legacy_ok = backup_factory != "UNKNOWN" and backup_factory == mac
            if legacy_ok:
                print(tr(
                    f"[WARNING] Legacy DEVICE_MAC.txt содержал runtime MAC {existing[1]}; factory ri@0x3e={mac}. Metadata будет исправлена.",
                    f"[WARNING] Legacy DEVICE_MAC.txt contained runtime MAC {existing[1]}; factory ri@0x3e={mac}. Metadata will be corrected.",
                ))
        if not legacy_ok:
            raise Error(tr(
    ...
    if family == "md":
        lines.extend([
            ...
            "identity_policy=md-factory-ri-v1",
        ])
    else:
        lines.extend([
...
def backup_tftp(
    ...
    allow_service_provisioning: bool = True,
) -> Path:
    ...
    try:
        ...
        _write_backup_device_identity(destination, telnet, model_name, detected_family)
        write_text(destination / "proc_mtd.txt", "\n".join(
            f'mtd{n}: {size:08x} {erase:08x} "{name}"' for n, (size, erase, name) in sorted(proc.items())
        ) + "\n")
        ...
        print(tr(
            "Один Telnet-сеанс с root-доступом использую для всех успешных разделов. Новый открою только после сбоя передачи или потери синхронизации.",
        ...
        for number in EXPECTED_NUMBERS:
                ...
                try:
                    ...
                    if detected_family == "mf" and number == 16:
                        ...
                        if not saved_stream_sha or saved_stream_sha != local_stream_sha:
                            print(tr(
                                "[ВНИМАНИЕ] Для сохранённого mtd16 нет проверенного SHA256 переданного потока. Сниму mtd16 заново, не сравнивая с меняющимся содержимым NAND.",
                            ...
                            print(tr(
                                f"[ГОТОВО] [{number + 1}/{len(EXPECTED_NUMBERS)}] Уже есть проверенный {target.name}; размер и SHA256 совпадают.",
                                f"[{number + 1}/{len(EXPECTED_NUMBERS)}] Existing validated {target.name} retained; gzip/size/transport SHA256 PASS.",
                            ))
                            continue
                    else:
                        print(tr(
                            f"[ГОТОВО] [{number + 1}/{len(EXPECTED_NUMBERS)}] Уже есть проверенный {target.name}.",
            ...
            for attempt in range(1, 4):
                ...
                try:
                    ...
                    print(tr(
                        f"[ШАГ] [{number + 1}/{len(EXPECTED_NUMBERS)}] Передаю mtd{number} ({name}) по TFTP, попытка {attempt}.{suffix}",
                        f"[{number + 1}/{len(EXPECTED_NUMBERS)}] TFTP mtd{number} ({name}), attempt {attempt}.{suffix}",
                    ))
                    print(tr(
                        f"[ЖДУ] mtd{number}: приём начат; исходный размер {_human_transfer_size(size)}.",
                    ...
                    while thread.is_alive() and time.time() < deadline:
                        ...
                        if done > 0 and (elapsed >= last_report + 15 or done >= last_bytes + 4 * 1024 * 1024):
                            print(tr(
                                f"[ЖДУ] mtd{number}: принято {_human_transfer_size(done)} сжатых данных, прошло {elapsed} с...",
                ...
                if attempt_error is not None or not marker_ok:
                    ...
                    if session_broken:
                        action_ru = "Сбойный сеанс закрыт; повторю только этот раздел в новом Telnet-сеансе с root-доступом."
                        action_en = "The unsynchronised session was closed; only this partition will be retried in a new root session."
                    else:
                        action_ru = "Telnet-сеанс синхронизирован; повторю только этот раздел в нём."
                        action_en = "The shell remains synchronised; only this partition will be retried in the current root session."
                    print(tr(
                        f"[ВНИМАНИЕ] TFTP mtd{number}, попытка {attempt} не завершена: {detail}. {action_ru}",
                ...
                try:
                    ...
                    print(tr(
                        f"[ВНИМАНИЕ] {exc}; Telnet-сеанс остаётся синхронизированным. Повторяю только mtd{number}.",
                ...
                partial.replace(target)
                print(tr(
                    f"[ГОТОВО] {target.name}; исходный размер {_human_transfer_size(size)}; сжатый {_human_transfer_size(target.stat().st_size)}; SHA256 {compressed_sha}",
                    f"[ГОТОВО] {target.name}; исходный размер {_human_transfer_size(size)}; сжатый {_human_transfer_size(target.stat().st_size)}; SHA256 {compressed_sha}",
                ))
                if detected_family == "mf" and number == 16:
                    write_text(
                    ...
                    print(tr(
                        f"[ГОТОВО] MF mtd16: SHA256 gzip-потока роутера совпал с файлом на ПК ({compressed_sha}).",
                        f"[ГОТОВО] MF mtd16: SHA256 gzip-потока роутера совпал с файлом на ПК ({compressed_sha}).",
                    ))
                    print(tr(
                        "[ВНИМАНИЕ] Повторный SHA256 всей NAND не использую как запрет: работающая Nokia может менять config/data/log во время копирования.",
...
_URSUS_SSH_SESSION_KEY: dict[str, Path] = {}
_URSUS_SSH_SESSION_KEY_MATERIAL: list[tuple[Path, str]] = []
...
def _ursus_session_keypair() -> tuple[Path, str] | None:
    """Create one ephemeral SSH key for this UrsusFlasher process.

    A production OpenWrt may require a root password. The system ssh client asks
    for it once; after that a throwaway key is installed for the remainder of
    this run so every following ssh/scp operation is deterministic. The private
    key stays in a temporary directory on the PC and is never logged.
    """
    if _URSUS_SSH_SESSION_KEY_MATERIAL:
        return _URSUS_SSH_SESSION_KEY_MATERIAL[0]
    keygen = shutil.which("ssh-keygen")
    if not keygen:
        return None
    try:
        directory = Path(tempfile.mkdtemp(prefix="ursusflasher-ssh-"))
        private = directory / "session_key"
        subprocess.run([keygen, "-q", "-t", "ed25519", "-N", "", "-C",
                        "ursusflasher-session", "-f", str(private)],
    ...
    if not public:
        return None
    _URSUS_SSH_SESSION_KEY_MATERIAL.append((private, public))
    return private, public


def _ursus_authorized_key_command(public: str) -> str:
    q = shlex.quote(public)
    return (
        "mkdir -p /etc/dropbear /root/.ssh; "
        f"(grep -qxF {q} /etc/dropbear/authorized_keys 2>/dev/null || echo {q} >> /etc/dropbear/authorized_keys); "
        f"(grep -qxF {q} /root/.ssh/authorized_keys 2>/dev/null || echo {q} >> /root/.ssh/authorized_keys); "
        "chmod 700 /root/.ssh 2>/dev/null; chmod 600 /etc/dropbear/authorized_keys /root/.ssh/authorized_keys 2>/dev/null; "
    )


def ensure_root_ssh_session(host: str, *, timeout: int = 180) -> tuple[int, str]:
    """Obtain a working root SSH session, allowing one normal password prompt."""
    # Reuse a verified ephemeral key when available.
    if host in _URSUS_SSH_SESSION_KEY:
        return ssh_run(host, "true", timeout=30, quiet=True, batch_mode=True)
    errors: list[str] = []
    for minimal in (True, False):
        try:
            rc, out = ssh_run(host, "id -u", timeout=30, quiet=True,
                              batch_mode=True, minimal_auth=minimal)
            if re.search(r"(?:^|\n)0(?:\n|$)", out.strip() + "\n"):
                _RESTORE_SSH_MINIMAL_AUTH[host] = minimal
                return rc, out
        except Error as exc:
            errors.append(str(exc).replace("\n", " ")[-500:])
    if not _console_can_prompt():
        raise Error("root SSH requires interactive authentication, but no console is available: " + " | ".join(errors)[-1200:])
    print(tr(
        f"[SSH] {host}: если root защищён паролем, сейчас его запросит системный OpenSSH. Пароль UrsusFlasher не сохраняет.",
        f"[SSH] {host}: if root is password-protected, system OpenSSH will ask now. UrsusFlasher does not store the password.",
    ))
    keypair = _ursus_session_keypair()
    command = "id -u"
    if keypair:
        command = _ursus_authorized_key_command(keypair[1]) + " id -u"
    rc, out = ssh_run(host, command, timeout=timeout, quiet=False, password_prompts=3)
    if not re.search(r"(?:^|\n)0(?:\n|$)", out.strip() + "\n"):
        raise Error("root UID 0 was not confirmed over SSH")
    if keypair:
        _URSUS_SSH_SESSION_KEY[host] = keypair[0]
        try:
            ssh_run(host, "true", timeout=30, quiet=True, batch_mode=True)
        except Error:
            _URSUS_SSH_SESSION_KEY.pop(host, None)
            raise Error("temporary SSH session key was installed but could not be verified")
...
def ssh_run(host: str, command: str, input_text: str | None = None, timeout: int = 900,
            ...
            password_prompts: int = 1) -> tuple[int, str]:
    ssh = ssh_executable()
    ...
    argv = [
        ...
        "-o", f"NumberOfPasswordPrompts={max(0, int(password_prompts))}",
    ]
    if batch_mode:
        argv.extend(["-o", "BatchMode=yes"])
    identity = _URSUS_SSH_SESSION_KEY.get(host)
    if identity and not minimal_auth:
...
def _uart_log_write(log, data: bytes, echo: bool = True) -> None:
    if data:
        log.write(data)
        log.flush()
        # Emergency recovery keeps the byte-accurate UART transcript in results/,
        # but the normal UI shows interpreted statuses instead of backend plumbing.
        if echo and os.environ.get("URSUS_QUIET_UART_UI") != "1":
...
def _uboot_prompt_present(data: bytes) -> bool:
    ...
    AN7581 Nokia builds may use ``AN7581>`` or the product prompt
    ``UrsusBoot>`` while the AN7583 OpenWrt reference U-Boot used by MF
    recovery has been hardware-observed to use ``U-Boot>``.  During prompt acquisition a Ctrl-C can already be in flight,
    so accept ``<INTERRUPT>`` immediately after the prompt as well and stop
    sending breaks at once.
    """
    tail = data[-2048:]
    named = re.search(
        rb"(?:^|[\r\n])(?:AN7581|AN7583|U-Boot|UrsusBoot)>[ \t]*(?=$|[\r\n]|<INTERRUPT>)",
        tail,
    )
...
def wait_uboot_prompt(serial_port: RecoverySerial, log, timeout: int = 180) -> str:
    ...
    while time.time() < deadline:
        ...
        if data:
            ...
            if b"lzma: res" in low or (b"failed to decompress image" in low and b"panic" in low):
                raise Error("RAM recovery helper failed before U-Boot (LZMA/PANIC). Full UART is saved in results/.")
            ...
            if b"starting kernel" in low or b"booting linux on physical cpu" in low:
                print("\n[UART][PRODUCTION_FALLBACK] Началась normal boot установленной системы; RAM prompt не захвачен. NAND не изменялась.")
                return "production"
...
def _rc23_timestamp_backup_identity_selftest() -> None:
    ...
    source = Path(__file__).read_text(encoding="utf-8")
    for token in (
        "_write_backup_device_mac(destination, telnet",
        "_stock_md_factory_mac",
        "_backup_md_factory_mac",
        "primary_mac_source=ri@0x3e",
        "identity_policy=md-factory-ri-v1",
        "stock eth0=",
    ):
        if token not in source:
            raise Error(f"fix11 MD backup identity token missing: {token}")
    if _validated_mac_bytes(bytes.fromhex("04123456789a")) != "04:12:34:56:78:9a":
        raise Error("fix11 MD factory MAC parser mismatch")
    if _validated_mac_bytes(bytes.fromhex("015665862e08")) != "UNKNOWN":
        raise Error("fix11 MD factory MAC parser accepted multicast identity")
    ...
    *,
    ...
    if choice == "1":
        try:
            ...
            print(tr(
                "[WARNING] Устройство может уже находиться в recovery/OpenWrt. Вернитесь в главное меню и выберите действие перехода на OpenWrt.",
                "[WARNING] The device may already be in recovery/OpenWrt. Return to the main menu and choose the OpenWrt transition action.",
            ))
        except Exception as exc:
...
def _firmware_capability_rows(family: str, variant: str, live_root: bool, geometry_ok: bool, backup_io_ok: bool = True) -> list[tuple[str, str]]:
    ...
    if family == "md":
        rows.extend([
            ...
            ("CAP_BOOTLOADER_REPLACE", "DEPRECATED bootloader-replacement research; not enabled"),
...
def service_menu() -> str:
    while True:
        ...
        if _INTERACTIVE_DESTRUCTIVE_LATCH.get("blocked"):
            print(tr(
                "[SAFETY-LATCH] Продолжение destructive stage заблокировано до успешного полного BootROM/UART recovery.",
                "[SAFETY-LATCH] Destructive-stage continuation is blocked until a successful full BootROM/UART recovery.",
            ))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command in (None, "wizard"):
            ...
            print("BootROM + bad-block restore + restore transport + RC23 timestamp/backup identity + RC24 interactive navigation + stage1 handoff + stock slot tolerance + read-only flow safety selftest: OK")
        rc = 0
