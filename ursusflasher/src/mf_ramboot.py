...
# HW-test packager updates these two values to the exact current green MF2 artifact.
FIP_SIZE = 319240
FIP_SHA256 = "1dd80a7729889bd719526bc3e3c44d948d4747853a3e98495cdbcadfbcd181b0"
...
def main() -> int:
    print("UrsusBoot-MF MF2 RAMBOOT / READ-ONLY HW TEST")
    ...
    verify(FIP, FIP_SIZE, FIP_SHA256, "UrsusBoot-MF MF2 RAM FIP")
    ...
    try:
        with log_path.open("ab", buffering=0) as log:
            ...
            print("Monitoring UART through WebFailsafe startup; the launcher will not stop at the first U-Boot banner...")
            ...
            print("\n")
            if version_seen and (board_seen or soc_seen):
                print("[PASS] UrsusBoot-MF MF2 identity observed on UART.")
            else:
                print("[CHECK REQUIRED] MF2 identity was not fully observed on UART.")
            ...
            if web_ready and http_ready:
                print("[PASS] WebFailsafe reached HTTP listen state at 192.168.1.1:80.")
                print("Continue hardware acceptance over LAN2/LAN3 without writing NAND.")
            else:
            ...
            return 0 if version_seen and (board_seen or soc_seen) and web_ready and http_ready else 2
    finally:
        serial_port.close()
